...
import { connectAccount, disconnectAccount, latestQrs, deleteSession, getGroups, extractGroupMembers, getGroupMembers, runNetworkProbe, sendMessage } from './modules/whatsapp/baileys.service';
...
export function setupRoutesLegacy(app: Express) {
  ...
  app.post('/api/campaigns/execute/:id', (req, res) => {
    ...
    try {
      ...
      try {
        accountIds = JSON.parse(campaign.accountIds || '[]');
      } catch (e) {
      ...
      const jobId = uuidv4();
      const accountId = accountIds[0]; // Use first account in campaign
      ...
      db.insert(queueJobs).values({
        ...
        status: 'pending',
        priority: 1,
        payload: JSON.stringify({ campaignId: id, accountId }),
        createdAt: new Date().toISOString()
      }).run();
      ...
      res.json({ 
        status: 'queued',
        jobId,
        message: `Campaign queued for execution. ${accountIds.length} account(s) will be used.`
      });
  ...
  app.get('/api/campaigns/:id/status', (req, res) => {
    ...
    try {
      ...
      const messageLogs_data = db.select()
        .from(messageLogs)
        .all()
        .filter((ml: any) => ml.campaignId === id);

      const sentMessages = messageLogs_data.filter((ml: any) => ml.status === 'sent').length;
      const failedMessages = messageLogs_data.filter((ml: any) => ml.status === 'failed').length;

      res.json({
        id,
        name: campaign.name,
        status: campaign.status,
        totalRecipients: (campaign.sentCount || 0) + (campaign.failedCount || 0),
  ...
  app.post('/api/auto-reply/logs', (req, res) => {
    try {
      const { ruleId, fromPhone, incomingMessage, autoReply, status } = req.body;
  ...
  app.get('/api/ai/keys', (req, res) => {
    try {
      ...
      const safeKeys = keys.map((k: any) => ({
        ...k,
        apiKey: k.apiKey ? k.apiKey.substring(0, 8) + '****' : '****'
      }));
  ...
  app.post('/api/ai/test-prompt', async (req, res) => {
    try {
      ...
      if (provider === 'openai') {
        ...
        const result = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKeyRecord.apiKey}`, {
          method: 'POST',
  ...
  app.get('/api/network/status', (req, res) => {
    try {
      ...
      const recentDiagnostics = allDiagnostics.filter((d: any) => new Date(d.checkedAt) >= startDate);

      const onlineTests = recentDiagnostics.filter((d: any) => d.status === 'online').length;
      const offlineTests = recentDiagnostics.filter((d: any) => d.status === 'offline').length;
      const totalTests = recentDiagnostics.length;

      const avgLatency = totalTests > 0 
        ? recentDiagnostics.reduce((sum: number, d: any) => sum + (d.pingMs || 0), 0) / totalTests
        : 0;

      const minLatency = totalTests > 0
        ? Math.min(...recentDiagnostics.map((d: any) => d.pingMs || 0))
        : 0;

      const maxLatency = totalTests > 0
        ? Math.max(...recentDiagnostics.map((d: any) => d.pingMs || 0))
        : 0;

      const uptime = totalTests > 0
        ? (onlineTests / totalTests) * 100
        : 100;

      let overallStatus: 'healthy' | 'degraded' | 'offline' = 'healthy';
      if (uptime < 95 || avgLatency > 200) overallStatus = 'degraded';
      if (uptime < 90 || avgLatency > 500) overallStatus = 'offline';

      // Get endpoint list from proxy profiles
      const proxies = db.select().from(proxyProfiles).all();
      const endpoints = proxies.map((p: any) => ({
        name: p.name,
  ...
  app.get('/api/network/diagnostics', (req, res) => {
    try {
      ...
      const allDiagnostics = db.select().from(networkDiagnostics).all();
      const recentDiagnostics = allDiagnostics
        .filter((d: any) => new Date(d.checkedAt) >= startDate)
        .map((d: any) => ({
          id: d.id,
  ...
  app.get('/api/network/latency', (req, res) => {
    try {
      ...
      // Group by time and calculate average latency
      const latencyByTime: Record<string, {latency: number, count: number}> = {};
      recentDiagnostics.forEach((d: any) => {
        const time = d.checkedAt.split('T')[0];
        if (!latencyByTime[time]) {
          latencyByTime[time] = { latency: 0, count: 0 };
        }
        latencyByTime[time].latency += d.pingMs || 0;
        latencyByTime[time].count += 1;
      });

      const latencyHistory = Object.entries(latencyByTime)
        .map(([time, data]) => ({
          time,
          latency: Math.round((data.latency / data.count) * 10) / 10
        }))
  ...
  app.get('/api/conversations/:id/messages', (req, res) => {
    try {
      ...
      // Simulate messages from messageLogs table
      const messages = db.select().from(messageLogs)
        .all()
        .map((log: any) => ({
          id: log.id || uuidv4(),
          conversationId: id,
          senderId: log.accountId || 'user',
          content: log.message || 'پیام',
