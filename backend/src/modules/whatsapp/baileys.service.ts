...
const forwardedMediaCache = new Map<string, any>();

/**
 * Send media message with retry mechanism for unstable connections.
 * It optimizes bandwidth by uploading the file once (to our own number)
 * and extracting the upload keys, or forwarding the previously uploaded message to others.
 */
export async function sendMediaMessage(accountId: string, toPhone: string, options: MediaSendOptions) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      const cacheKey = `${accountId}:${preparedMedia.absoluteMediaPath}`;
      let cachedProto = forwardedMediaCache.get(cacheKey);

      if (!cachedProto && sock.user?.id) {
        logger.info(`Uploading media to self to cache for forwarding: ${preparedMedia.absoluteMediaPath}`);
        const selfJid = sock.user.id.split(':')[0] + '@s.whatsapp.net';
        
        // Upload without caption for the cached structure
        const uploadContent = { ...preparedMedia.messageContent };
        delete uploadContent.caption;
        
        const uploadedMsg = await sock.sendMessage(selfJid, uploadContent);
        if (uploadedMsg) {
          cachedProto = uploadedMsg;
          forwardedMediaCache.set(cacheKey, cachedProto);
        }
      }

      if (cachedProto) {
         // Use Baileys forward functionality and add the caption for this specific user
         logger.info(`Forwarding cached media to ${jid}`);
         await sock.sendMessage(result.jid, { 
             ...
             caption: preparedMedia.messageContent.caption // Re-add dynamic personalized caption
         });
      } else {
         // Fallback if uploading to self failed somehow
